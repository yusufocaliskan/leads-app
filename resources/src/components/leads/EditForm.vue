  ...
  <div v-if="show == true " class="mt-10 sm:mt-0 bg-gray-50">
    ...
    <div class="md:col-span-1">
        <div class="px-4 sm:px-0">
          <h3 class="text-2xl font-medium leading-6 text-gray-900">{{lead_data.name}}</h3>
          <p class="mt-1 text-sm text-gray-600">
              <a class="text-indigo-700 hover:underline" href="mailto:{{lead_data.email}}">{{lead_data.email}}</a>
          </p>
        </div>
      </div>
            ...
            <div class="px-4 py-5 bg-white sm:p-6">
              ...
              <div class="grid grid-cols-6 gap-6">
                ...
                <div class="col-span-6 sm:col-span-3">
                  ...
                  <input v-model="lead_data.name" type="text" name="name" id="name" autocomplete="given-name" class="mt-1 p-2 border-1 focus:ring-indigo-500 border-zinc-700 focus:border-indigo-500 block w-full shadow-sm sm:text-sm  rounded-md" />
                </div>

                <div class="col-span-6 sm:col-span-3">
                  <label for="email" class="block text-sm font-medium text-gray-700">E-mail</label>
                  <input v-model="lead_data.email" type="text" name="email" id="email" autocomplete="family-name" class="mt-1 p-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm  rounded-md" />
                </div>
               ...
               <div class="col-span-8 text-red">
                   The lead  <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded text-emerald-600 bg-emerald-200 uppercase last:mr-0 mr-1">
                        {{lead_data.terms ? 'approves' : lead_data.terms}}
                    </span> terms policy
               </div>
                <input type="hidden" name="_id" v-model="lead_data._id">
                <input type="hidden" name="terms" v-model="lead_data.terms">
              </div>
            </div>
