          ...
          <div class="flex items-center">
            ...
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                <router-link :to="item.href" v-for="item in navigation" :key="item.name"  :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-500  hover:text-gray-900', 'px-3 py-2 rounded-md text-sm font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</router-link>
              </div>
            </div>
          </div>
          <div class="hidden md:block">
